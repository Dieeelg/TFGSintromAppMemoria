...
No caso de Azure Document Intelligence, o cliente utilizado pola aplicación substitúese por un \textit{MagicMock}. Desta maneira pódense construír respostas coa mesma estrutura que as devoltas polo servizo e comprobar posteriormente como son procesadas pola \gls{api}. A preparación deste cliente simulado móstrase no Listado~\ref{lst:mock-azure}.
...
Como exemplo, o Listado~\ref{lst:api-control-data} comproba a corrección da data correspondente a un control cando a data detectada non coincide coa próxima visita indicada na folla.
...
As chamadas ao \gls{sdk} de Firebase simuláronse durante as probas automatizadas. Isto permite comprobar a información empregada para construír a notificación, o token de destino e o tratamento dos posibles erros sen enviar mensaxes reais. O Listado~\ref{lst:mock-firebase} mostra un dos casos empregados para verificar este comportamento.
...
A comunicación \gls{http} probouse substituíndo o cliente real por un \textit{MockClient}. Deste modo é posible interceptar as peticións xeradas pola aplicación e comprobar o método \gls{http} empregado, a ruta, as cabeceiras e os datos enviados. O Listado~\ref{lst:flutter-mock-http} mostra unha destas comprobacións.
...
O Listado~\ref{lst:flutter-reimportacion} comproba que unha toma rexistrada fóra da hora prevista se conserva despois de importar novamente a mesma pauta.
...
Comprobouse que os datos poden descifrarse coa clave correspondente e que unha mensaxe modificada, un tipo de aviso diferente ou unha clave incorrecta provoca o rexeitamento da mensaxe. Unha destas comprobacións recóllese no Listado~\ref{lst:flutter-cifrado}.
...
Comprobáronse diferentes estados das tomas e verificouse que unha toma xa confirmada non xere novos avisos. O Listado~\ref{lst:flutter-planificador} mostra un dos casos empregados para comprobar esta planificación.
...
As probas da \gls{api} executáronse desde a raíz do repositorio empregando Coverage.py, mediante os comandos recollidos no Listado~\ref{lst:execucion-probas-api}.

\begin{lstlisting}[
    caption={Execución das probas e cálculo da cobertura da API.},
    label={lst:execucion-probas-api}
]
...
No cliente empregouse o comando de probas de Flutter coa xeración simultánea do ficheiro LCOV, como se mostra no Listado~\ref{lst:execucion-probas-flutter}.

\begin{lstlisting}[
    caption={Execución das probas automatizadas de Flutter.},
    label={lst:execucion-probas-flutter}
]
flutter test --no-pub --coverage
\end{lstlisting}

A suite executou correctamente 114 probas. O ficheiro LCOV xerado contabilizou 2447 liñas instrumentadas, das cales 1781 foron executadas, o que representa unha cobertura global do 72,8\,\%. A Figura~\ref{fig:execucion-probas-flutter} mostra o resultado desta execución.
...
%===================================================================================================================================================
\section{Comprobacións en dispositivo}
%===================================================================================================================================================
...
Tamén se comprobou a recepción real dos avisos xerados pola aplicación no sistema de notificacións de Android, como se mostra na Figura~\ref{fig:proba-notificacion}.
...
Finalmente, a sincronización entre os dous roles comprobouse realizando a toma dunha dose no dispositivo do paciente e verificando posteriormente a actualización da información no dispositivo da persoa supervisora. O resultado desta comprobación móstrase na Figura~\ref{fig:proba-sincronizacion}.
...
\begin{figure}[htbp]
    ...
    \includegraphics[
        ...
        trim={0 10cm 0 0},
        clip
    ]{imaxes/09-Probas/anexo/toma-confirm-sup.jpg}
