...
final api = ServizoApi(
  client: MockClient((request) async {
    ...
    expect(request.url.path,
        ConstantesAplicacion.endpointEnviarNotif);
...
A \gls{api} presenta unha cobertura elevada e uniforme. As liñas que permanecen sen executar concéntranse no módulo de preprocesamento, que contén numerosas ramas destinadas a tratar diferentes características e posibles problemas das imaxes de entrada.

No cliente tamén se acadou unha cobertura elevada nos modelos, no acceso á \gls{api}, na persistencia e no cifrado. A cobertura global é menor debido principalmente ás partes relacionadas coa interface e ás funcionalidades que dependen directamente do dispositivo, como Firebase Messaging ou o sistema de notificacións locais.
