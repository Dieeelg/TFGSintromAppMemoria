...
Como resultado, a ferramenta devolvía o texto detectado xunto coa súa organización en bloques, liñas e elementos. Esta información gardouse en formato \gls{json} para facilitar a análise posterior da orde na que se devolvían os diferentes elementos do documento.
...
No caso da aproximación baseada en \gls{yolo}, valorouse adicionalmente a capacidade de localizar as representacións gráficas das pastillas e a posibilidade de utilizar esas deteccións como punto de partida para delimitar a información da cela correspondente.
...
Un dos exemplos máis representativos obsérvase na folla F01. Como mostra o Listing~\ref{lst:tesseract-estandar-f01}, a saída conserva algúns elementos recoñecibles, pero introduce numerosos caracteres incorrectos e perde a estrutura necesaria para asociar de forma fiable cada data coa súa dose.
...
Figura~\ref{fig:tesseract-sintromv2-adestramento}, o \gls{bcer} pasou de valores próximos ao 39\,\% nas primeiras iteracións ata alcanzar un mínimo do \textbf{10,878\,\%}.
...
\subsection{Resultados da aproximación baseada en YOLO}

O detector mostrou un rendemento elevado no conxunto de validación. A curva Precision-Recall alcanzou un \gls{map05} de \textbf{0,989} e a curva F1 acadou un valor máximo de \textbf{0,98} cun limiar de confianza de \textbf{0,603}. Na matriz de confusión rexistráronse \textbf{130 deteccións correctas} da clase \textit{pastilla} e \textbf{5 falsos positivos}.

\begin{figure}[htbp]
    \centering
    \begin{subfigure}[t]{0.4\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imaxes/06-Procesamento automático das follas de tratamento/BoxPR_curve.png}
        \caption{Curva Precision-Recall.}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.5\textwidth}
        \centering
        \includegraphics[width=\linewidth]{imaxes/06-Procesamento automático das follas de tratamento/confusion_matrix.png}
        \caption{Matriz de confusión.}
    \end{subfigure}

    \caption{Resultados obtidos durante a validación do detector YOLO.}
...
A combinación de ambas técnicas permitiu comprobar que a detección da representación gráfica pode empregarse para aproximar a posición da información de interese, pero non permite establecer de maneira suficientemente fiable os límites da cela nin garantir a asociación entre unha data e a súa dose. Ademais, as entradas como \textit{0},  textit{NO TOMAR} ou \textit{CONTROL} non conteñen a representación gráfica empregada como punto de partida polo detector.
...
A plataforma de Azure devolve a información extraída nun documento con formato \gls{json} para que poida ser consumido. O problema subxacente desta mala lectura visual é que o \gls{json} resultante herda todo este caos. Ao non seguir un patrón predicible na súa resposta (nalgúns casos o \gls{json} devolve a data e a dose xuntas, noutros devólveas separadas en distintas filas do obxecto …), resulta practicamente imposible deseñar unha lóxica de extracción que sexa capaz de recoller a información correcta para o día adecuado de forma robusta e automática
...
\subsection{Resultados do preprocesamento}
