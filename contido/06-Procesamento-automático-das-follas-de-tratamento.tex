...
A mellora non eliminou, con todo, os problemas de recoñecemento. Nas follas F09 e F10 continuaron aparecendo modificacións nas datas e perda da estrutura da táboa. Por exemplo, en F10 obtivéronse valores como \textit{068/09/2025} en lugar de \textit{08/09/2025} ou
\textit{13/10/2023} en lugar de \textit{13/10/2025}, ao mesmo tempo que os elementos da pauta aparecían parcialmente desordenados.
...
A avaliación de Azure AI Document Intelligence realizouse comparando os dous modelos personalizados adestrados, \textit{Template} e \textit{Neural}. Primeiro analizáronse as métricas proporcionadas por Azure Document Intelligence Studio ao finalizar o adestramento e, posteriormente, avaliáronse ambos modelos sobre as follas reservadas para as probas.

No modelo \textit{Template}, mostrado na Figura~\ref{fig:azure-metricas-template}, obtéñense valores elevados na maior parte dos campos: 99,50\,\% para \textit{fecha visita}, \textit{inr}, \textit{farmaco oral} e
\textit{prox visit}, 98,80\,\% para \textit{DOSE} e 94,70\,\% para \textit{RUV}. Os valores máis baixos corresponden a \textit{dosis semanal} e \textit{centro visita}, cun 87,50\,\%, e a \textit{turno}, cun 62,50\,\%. . Non obstante, estes campos teñen unha relevancia secundaria dentro da aplicación, xa que non afectan ao procesamento principal. En consecuencia, aínda que estes resultados son inferiores aos do resto dos campos, non representan un impacto significativo sobre o rendemento global do sistema.

No modelo \textit{Neural}, Azure Document Intelligence non mostra valores de precisión por campo durante o adestramento. A diferenza do modelo \textit{Template}, no que estas métricas se calculan a partir da comparación entre os campos etiquetados e os valores extraídos, o modelo \textit{Neural} non proporciona esta estimación na fase de adestramento. Por este motivo, os campos aparecen sen valor na columna de precisión do Studio, sen que isto implique un erro no modelo.
...
O modelo \textit{Template} non estivo exento de erros. Por exemplo, na folla F09 o campo \textit{turno} foi asociado incorrectamente co valor \textit{C.S. MONDOÑEDO}, mentres que o modelo \textit{Neural} recuperou correctamente \textit{Mañanas}. Isto tamén concorda co 62,50\,\% mostrado polo Studio para "turno" pero como comentábamos antes estes campos non supoñen un 


Podemos observar que nas condicións con menos luz a casilla de contro resultalle algo complicada de extraer correctamente,a lgunhas veces non extrae a data por exemplo, a entrada \textit{CONTROL 05 ABR} quedou reducida a \textit{CONTROL}. Isto non resulta un problema para a aplicaciópn xa que xustoi debaixo da táboa temos un campo coa data da próxima visita que conta cun \,\% polo que a data se pode obter a aprtir del.

% Unha conslusión breve sen adiantar moito antes da táboa de resultados 
