...
\label{subsec:resumo-comparativo-procesamento}

Co obxectivo de facilitar a lectura dos resultados, a
Táboa~\ref{tab:comparacion-metodos-extraccion} resume o comportamento de
cada aproximación nos elementos que debe recuperar a aplicación. As
valoracións son cualitativas e sintetizan as probas descritas nas seguintes
subseccións; non constitúen unha comparación directa de taxas de acerto, xa que
as técnicas producen saídas diferentes e non todas foron avaliadas sobre o
mesmo subconxunto de imaxes.
...
\begin{table}[htbp]
    ...
    \caption{Resumo cualitativo dos resultados das técnicas de extracción.}
    ...
    \begin{tabularx}{\textwidth}{
        ...
        >{\centering\arraybackslash}X}
        \toprule
        \textbf{Método} & \textbf{Texto} & \textbf{Datas} & \textbf{INR} &
        \textbf{Doses} & \textbf{Relación data--dose} & \textbf{Fotos} \\
        \midrule
        ML Kit
            & Parcial & Mal & Mal & Mal & Mal & Mal \\
        Tesseract estándar
            & Parcial & Mal & Mal & Mal & Mal & N/A \\
        Tesseract \textit{sintromv2}
            & Parcial & Mal & Parcial & Parcial & Mal & Mal \\
        YOLO + Tesseract
            & N/A & Mal & N/A & Parcial\textsuperscript{a} & Mal & Parcial \\
        Azure \textit{Neural}
            & Ben & Ben & Ben & Ben & Parcial & Parcial \\
        \textbf{Azure \textit{Template}}
            & \textbf{Ben} & \textbf{Ben} & \textbf{Ben} & \textbf{Ben} &
              \textbf{Ben} & \textbf{Ben\textsuperscript{b}} \\
        \bottomrule
    \end{tabularx}
    ...
    \begin{minipage}{0.98\textwidth}
        \tiny
        \textit{Lenda:} \textit{Ben}, resultado suficientemente estable nas
        probas; \textit{Parcial}, recuperación posible pero non fiable en todos
        os casos; \textit{Mal}, os erros impiden reconstruír a pauta;
        \textit{N/A}, campo alleo ao obxectivo específico da técnica.
        O método seleccionado destácase en negra.
        \textsuperscript{a}YOLO detecta con elevada fiabilidade as
        representacións gráficas das pastillas, pero non as doses expresadas
        unicamente mediante texto.
        \textsuperscript{b}Detectáronse erros puntuais nas condicións menos
        favorables, polo que a aplicación permite revisar e corrixir a extracción.
    \end{minipage}
\end{table}

O resumo mostra que recoñecer texto ou localizar unha pastilla non é
suficiente: o criterio determinante é conservar a correspondencia entre cada
data e a súa dose. O modelo \textit{Template} de Azure foi o único que mantivo
esta estrutura coa estabilidade necesaria para empregalo como base da
solución, aínda que a saída continúa sometida á revisión da persoa usuaria.

Para evitar duplicar material, a Táboa~\ref{tab:guia-exemplos-procesamento}
identifica os exemplos de entrada e resultado incluídos ao longo da sección.
Estes casos permiten contrastar unha fotografía de uso real, follas non vistas
durante os adestramentos e unha captura inclinada.

\begin{table}[htbp]
    \centering
    \footnotesize
    \renewcommand{\arraystretch}{1.2}
    \caption{Guía dos exemplos empregados para presentar os resultados.}
    \label{tab:guia-exemplos-procesamento}
    \begin{tabularx}{\textwidth}{
        >{\raggedright\arraybackslash}p{2.7cm}
        >{\raggedright\arraybackslash}p{3.1cm}
        >{\raggedright\arraybackslash}X}
        \toprule
        \textbf{Técnica} & \textbf{Folla e condición} &
        \textbf{Resultado mostrado} \\
        \midrule
        OpenCV
            & T1, fotografía inclinada
            & Figura~\ref{fig:opencv-preprocesamento}: comparación antes e
              despois da corrección de orientación e perspectiva. \\
        ML Kit
            & F01, iluminación normal
            & Figura~\ref{fig:mlkit-f01n-entrada} e
              Listado~\ref{lst:mlkit-f01n-saida}: perda da estrutura e erros en
              datas e fraccións. \\
        Tesseract estándar
            & F01, imaxe de referencia
            & Listado~\ref{lst:tesseract-estandar-f01}: caracteres incorrectos
              e asociación data--dose non fiable. \\
        Tesseract \textit{sintromv2}
            & F11, non empregada no adestramento
            & Figura~\ref{fig:tesseract-comparacion-f11}: comparación directa
              co modelo estándar e mellora no recoñecemento das fraccións. \\
        YOLO e YOLO + Tesseract
            & F10, non empregada no adestramento
            & Figuras~\ref{fig:yolo-f10n} e
              \ref{fig:yolo-tesseract-recortes}: detección correcta das
              pastillas, pero recortes insuficientes para reconstruír a cela. \\
        Azure \textit{Neural}/\textit{Template}
            & F10, iluminación normal
            & Figuras~\ref{fig:azure-comparacion-f10n-1} e
              \ref{fig:azure-comparacion-f10n}: o modelo \textit{Template}
              conserva mellor a agrupación de datas e doses. \\
        \bottomrule
    \end{tabularx}
\end{table}
